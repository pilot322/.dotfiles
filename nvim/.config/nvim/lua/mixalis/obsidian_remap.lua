vim.keymap.set('n', '<leader>obss', function()
    ...
    local command = string.format('!rclone bisync -v %s ObsidianNotes:%s', cwd, folder_name)
    vim.cmd(command)
...
vim.keymap.set('n', '<leader>obsr', function()
    ...
    local command = string.format('!rclone bisync --resync -v %s ObsidianNotes:%s', cwd, folder_name)
    vim.cmd(command)
end, { noremap = true, silent = true, desc = 'Run terminal command with current directory' })
